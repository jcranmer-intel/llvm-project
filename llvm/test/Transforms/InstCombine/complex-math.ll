...
; RUN: opt < %s '-passes=instcombine<max-iterations=2>' -S -inst-combine-complex | FileCheck %s
...
;
  ...
  %x = fsub double %ac, %bd
  %y = fadd double %ad, %bc
  %dest.real = getelementptr %complex.double, ptr %dest, i64 0, i32 0
...
; CHECK: attributes #[[ATTR2]] = { "complex-range"="no-nan" }
;.
